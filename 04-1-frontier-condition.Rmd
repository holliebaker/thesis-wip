...
The algorithm from Section \@ref(sec:monotone-algorithm) produces such a CAD, which is also sign-invariant on a set of polynomials $\mathbf{F} \subset \Z[x_1,\ldots,x_n]$. It is clear that $\mathbf{F}$ should contain the polynomials in defining formulas for sets $V_i$, along with some additional polynomials which ensure that each cell $C \subset V_1 \cup \ldots \cup V_k$ monotone.
In this section, we describe how to refine $\mathcal{D'}$ in order that it satisfies the frontier condition. A refinement, $\mathcal{D''}$ of $\mathcal{D'}$ satisfies the frontier condition if $\fr{C}$ is a union of cells of $\mathcal{D''}$, for each cell $C \subset V_1 \cup \ldots \cup V_k$ of $\mathcal{D'}$.
...
$$
Consider $\fr{W}$, which can be expressed as the disjoint union of four semialgebraic sets:
\begin{align*}
\fr{W} =& \{ (x,y,z) \in \R^3 \mid 0 < x < 1, y = x, z = 1 \}, \\
& \{ (x,y,z) \in \R^3 \mid 0 < x < 1, y = -x, z = 1 \}, \\
& \{ (x,y,z) \in \R^3 \mid x = 1, -1 < y < 1, z = y^2 \}, \\
& \{ (0,0,z) \in \R^3 \mid 0 \le z \le 1 \}.
\end{align*}
...
Note that $W$, from Example \@ref(exm:whitney-jhd), is a $(1,1,0)$-cell, but it is not monotone. In fact, $W$ is not even the graph of a quasi-affine map, since $\projops{x,z}(W)$ is two-dimensional, but $\projops{x,z}\vert_W$ is not injective. The blow-up subset $B$, contained in $\fr{W}$ means that $W$ is not a topologically regular cell -- a weaker property than being monotone.
...
## Construction of a CAD with frontier condition, due to @bgv15, Theorem 3.20 { #sec:frontier-bgv }

Given the monotone CAD $\mathcal{D'}$ computed in the algorithm from Section \@ref(sec:monotone-algorithm), our final step, according to [@bgv15, Theorem 3.20] is to compute a refinement $\mathcal{D''}$ of $\mathcal{D}$ such that the frontier $\fr{C} = \cl{C} \setminus C$ of each $2$-dimensional cell $C \subset V_1\cup \ldots, \cup V_k$ is a union of cells of $\cal D'$.

The proof of @bgv15, Theorem 3.20 proceeds as follows. For each cell $C \subset V$, compute the the semialgebraic set $\fr{C}$. Since $C$ is monotone, we can assume that $\fr{C}$ is homeomorphic to a circle. Construct a partition $\cal U$ of $\fr{C}$ into monotone one-dimensional curves and points, such that $\cal U$ is compatible with all one-dimensional cells of $\cal D'$.
...
The most computationally difficult part of implementing this construction is finding $\fr{C}$ for each $2$-dimensional cell $C \subset V$.
...
We see that the "obvious" way of computing the frontier involves solving a quantifier elimination problem with two quantifier alternations. Using an efficient algorithm, e.g., [@bpr2006, Algorithm 14.21], this procedure has complexity singly exponential in the number of variables. $\partial C$ is represented by a quentifier-free Boolean formula $F$.
...
In the following section, we will explore and alternative way to, based on an algebraic construction described by @lazard10, obtain the frontier condition.
...
The proof of @lazard10, Proposition 5.14 depends on taking Zariski closures. There are now defined and a relevant property presented, closely following @cox2013.
...
- For each $(0,0)$-cell $\mathbf{c}$ of $\cal D'$, determine whether it is a bad cell.
  ...
  $\mathbf{b}$ is a bad cell if there exists a polynomial $f \in {\cal A}_3$ such that $f$ has constant sign on a $(0,0,1)$-cell $C$ of $\cal D$ such that $\mathbf{p} = \projop{2}(C)$.
...
- If $\mathbf{b}$ is bad:
  ...
  - For each $g \in {\cal A'}$, do
    ...
    - Compute
    $$
    L := \{ \rm{sub}_{\mathbf{b}}(f') \in \Q[x_3] \mid f' \in S \}
    $$
...
Every $2$-dimensional cylindrical cell in a CAD of $\R^n$ is the graph of a continuous definable map $\mathbf{f} = (f_1,\ldots,f_{n-2}) : X \to \R^{n-2}$ where $X \subset \opspan{x_i,x_j}, 1 \le i < j \le n$ in a cylindrical $(1,1)$-cell by Proposition \@ref(prp:delete-zero-index) (@bgv13, Lemma 3.3). According to Remark \@ref(rem:blow-up-codimension), each component $f_j : X \to \opspan{x_j}$ of $\mathbf{f}$ is a continuous definable map at every point in $\cl{X}$ except for a finite set of isolated points in $\fr{X} = \cl{X} \setminus X$.
...
If $C'_1$ is the projection of a $1$-dimensional cell contained in $\fr{C}$, we are done -- the boundary of this one-dimensional cell consists of some isolating points including $\mathbf{p}$.
...
By Proposition \@ref(prp:delete-zero-index) (@bgv15, Lemma 3.3), the images of $C$, $D$ and $E$ under the projection map $\projops{i_1,i_k,i_n}$ are cylindrical cells and satisfy the the three-dimensional case of Lemma \@ref(lem:lazard-5-13) (@lazard10, Lemma 5.13).
:::
...
be a common root of $g_d,\ldots,g_1,g_0$.
Let $g_1,\ldots,g_{n-1}\in \Q[x_1,\ldots,x_{n-1}]$ be irreducible, pairwise relatively prime polynomials having $\mathbf{p}$ as a root.
...
$\cl{C}$ is the graph of $\varphi$ for all but a finite number of isolated blow-up points in $\cl{C'}$ (see Remark after Definition \@ref(def:blow-up-point)).
...
Apply a method similar to that described in @lazard10, Algorithm 5.15 to $C$ and $D$. Usince Conjuecture \@ref(cnj:ext-lazard-5-14), compute the saturations by $g_{\ell_1}$ and $g_{\ell_2}$, respectively:
$$
...
- For each $(i_1,\ldots,i_{k-1},0)$-cell $E \in \cal{C}$, let $i_{\ell_1} = i_{\ell_2} = 1$, while all other elements in the index are equal to zero.
  ...
  - Let $\mathbf{c} := \projop{j_{\ell_1} - 1}}(E)$ and compute the set of polynomials for the sub-cad of $\mathcal{D}$ above $\mathbf{c}$.
    ...
    $$
    \mathcal{A'}_{j} := \{ \rm{sub}_{\mathbf{c}} (f) \in \Z[x_{\ell_1},\ldots,x_j] \mid f \in \mathcal{A}_{i_{\ell_1} - 1 + j} \}
    $$
  ...
  - For each $D$ projecting on a bad cell, let $\mathbf{b} := \projop{k-1}(D)$.
    ...
    $$
    L := \{ \rm{sub}_{\mathbf{b}}(f) \in \Q[x_k] \mid f \in S_1 \cup S_2 \}.
    $$
