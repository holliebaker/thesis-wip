...
1. Let $\mathbf{F} \subset \Z[x_1,\ldots,x_n]$
be the set of polynomials appearing in the formulas $F_1,\ldots,F_k$.

1. Compute $\cal E$: an $\mathbf{F}$-invariant CAD of $\R^n$ using the McCallum projection operator. This operation is guaranteed to succeed since each $V_1,\ldots,V_k$ has dimension at most two and, therefore $\mathbf{F}$ is well-oriented.

1. Let ${\cal A} = ({\cal A}_1,\ldots,{\cal A}_{n})$ be the family of projection polynomials for $\cal E$, where each ${\cal A}_k \subset \Z[x_1,\ldots,x_k]$.

1. For each cell $C \subset V$

    do:

    - Observe that the binary index $(i_1,\ldots,i_n) \in \{0,1\}^n$ of $C$ contains $k \le 2$ elements $i_{j_1}, \ldots, i_{j_k}$ which are equal to $1$, while all other elements of the binary index are equal to $0$.

    - For each $i \in \{1,\ldots,n\} \setminus \{j_1,\ldots,j_k\}$, find a polynomial in ${\cal A}_i$ which is equal to $0$ at every point $\mathbf{x} \in C$. This can easily be done by checking the set of signs of projection factors associated with $C_k$.

1. For each $2$-dimensional cell $C_2 \subset V$ and each pair $1 \le i_1 < i_2 \le n$,

    do:

    - Find the set of polynomials $f_1,\ldots,f_{n-2}$ defining $C_2$ as described above.

    - Define the $((n-2) \times (n-2)$ Jacobi matrix
    \begin{equation}
    J_{C_2,\hat{i_1 i_2}} :=
...
(\#eq:jacobi-two)
    ...
    - $\det(J_{C_2,\hat{i_1 i_2}}) = 0$ at critical points of $\projops{i_1,i_2}$.

1. For each $1$-dimensional cell $C_1 \subset V$ and each $1 \le i \le n$:

    do:

    - Find the set of polynomials $f_1,\ldots,f_{n-1}$ defining $C_1$.

    - Define the $((n-1)\times (n-1))$ Jacobi matrix
  \begin{equation}
  J_{C_1,\hat{i}} :=
...
(\#eq:jacobi-one)
  \end{equation}
    ...
    - $\det(J_{C_1,\hat{i}}) = 0$ at the critical points of $\projops{i}$.

-  Compute $\cal D$, an $\mathbf{F`}$-invariant CAD of $\R^n$, for
$$
