...
More precisely, according to @mccallum1988, Theorems 2.2.3 and 2.2.4, if there is a finite number of blow-up points
...
Regular points of the projection of $W_k'$ onto $\opspan{j_1,\ldots,j_{n - \ell}}$ are those at which the matrix $J_{i_1,\ldots,i_\ell}$ has full rank.
...
On the other hand, $J_{\{1,\ldots,n\} \setminus \{i\}}$ will be an $((n-2) \times (n-1))$-matrix. It will fail to have
full rank if the determinants of all of the $((n-2) \times (n-2))$-minors are zero. Observe that the minors of
$J_{\{1,\ldots,n\} \setminus \{i\}}, i \in \{1,\ldots,n\}$ are the $((n-2)\times (n-2))$-Jacobi matrices
$J_{\{1,\ldots,n\} \setminus \{i,j\}}, j \in \{1,\ldots,n\} \setminus \{i\}$. I.e., they will coincide with the square matrices associated with $W'_{i,j}$.
Thus, the CAD should have constant sign on each of the determinants
$$
\det(J_{\{1,\ldots,n\} \setminus \{i,j\}}) \text{ for all } 1 \le i < j \le n,
$$
...
$0$-dimensional critical points of $\projops{x_i}$ and $\projops{x_j}$.
...
- For each cell $C_k \subset V$ with binary index $(i_1,\ldots,i_n) \in \{0,1\}^n$, such that $i_{j_1} = \ldots =
  i_{j_k} = 1$ while all other elements of the binary index are equal to $0$ (since $\dim(V) \le 2$, $k \le 2$ and $\dim(C_k) = k$):
...
(\#eq:jacobi-one)
  ...
  - Construct $\cal D$, an $\mathcal{F'}$-invariant CAD of $\R^n$, where
    \begin{align*}
    \mathbf{F}' =\ & & \mathbf{F}\\
...
The algorithm then makes the CAD compatible with the determinants of matrices $J_{C_2,\mathbf{m}}$ and
$J_{C_1,\mathbf{m}}$, defined in Equations \@ref(eq:jacobi-two) and \@ref(eq:jacobi-one) respectively. These are the
...
critical points of projections of the smooth $2$-dimensional locus of $V_1\cup \ldots \cup V_k$ onte one- and $2$-dimensional coordinate subspaces, as required in the proof of @bgv15, Lemma 3.19.
...
$$
The critical points of $1$- and $2$- dimensional strata are then computed. This is done by computing determinants of $(n-1)$- and $(n-2)$-dimensional Jacobi matrices, containing the $n-1$ and $n-2$ polynomial equations which define the strata of codimension $1$ and $2$ respectively.
