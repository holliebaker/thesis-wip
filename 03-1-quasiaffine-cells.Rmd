...
n (d + 1)^{1^{O(n)}}.
$$
For the critical points of $\projops{x_i}, 1 \le i \le n$, there will be $n$ Jacobi determinants, and for $\projops{x_i,x_j}, 1 \le i < j \le n$, there will be $(n-1) + (n-2) + \cdots + 1 = 1/2 (n-1)n$, which is asymptotically the same as $n^2$. 

When a "classical" CAD, having constant sign on the $s$ input polynomials plus the $n + n^2$ Jacobi determinants, we have a complexity, according to @bpr2006, Algorithm 11.2, of
$$
((s + n^2 + n) (n^2 (d + 1)^{1^{O(n)}}))^{O(1)^{n}}.
$$
