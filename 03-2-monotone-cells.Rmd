...
$$
Although the cell $X$ under consideration is a $2$-dimensional cylindrical cell in $\opspan{x_1,x_\alpha}$, we actually need to refine the $2$-dimensional cylindrical sector cell $C' := \projop{\alpha}(C)$ in the cylindrical decomposition induced by $\cal D$ on $\R^{\alpha}$. Thus, we need a way to pass from $C'$ and $X$. 

:::{.lemma #gamma-top-bottom}
Let $C$ be a $(1,0,\ldots,0,1)$-cell, $X := \projops{x_1,x_\alpha} (C)$ and $X' := \plojop{1}(C)$. Suppose we have the family ${\cal I}(c), c \in X'$ and set of one-dimensional curves associated with $C$. Then
...
Observe that the graph of $f$ (if finite) is the bottom of $C$ and the graph of $g$ (if finite) is the graph of $g$.
