...
- If $n \le 2$,
    ...
    - Return immediately: all cells in the CAD are already monotone.

    End if.

- For each $(0)$-cell $\mathbf{c}$ of the decomposition induced by $\mathcal{D}$ on $\R^1$, which is the projection of a
  *true* cell of $\cal D$ of dimension $2$,
    ...
    - Let $\mathcal{D'}$ be the sub-CAD of $\mathcal{D}$ in $\R^{n-1}$ above $\mathbf{c}$.

    - Proceed by induction by calling
    $$
    \mathrm{MonotoneCadHelper}(D', A', \mathbf{c}).
    $$
...
- For each $(1)$-cell $C''$ of the decomposition induced by $\mathcal{D}$ on $\R^1$ which is the projection of a *true*
  cell of $\cal D$ of dimension $2$,

    do:

    - For each *true* cell $C$ of $\mathcal{D}$ such that $\dim(C) = 2$ and $\projop{1} = C''$,

        do:
        - Observe that the cell $C$ has binary index $(1,0,\ldots,0,i_\alpha,0,\ldots,0)$ where $i_\alpha = 1$.
...
\end{pmatrix},
    $$
                from Equation \@ref(eq:jacobi-det), using constraints $g_{2},\ldots,g_{j-1}$, and optimisation function $g_j$.


                - let $d_\delta := \partial g_j / \partial x_j$
                These denominators of the total derivatives of $g_j$ in the last column will be delt with later.
...
- Let $\mathcal{R}_\mathbf{b}$ be the union of the real roots of polynomials in $\mathcal{F}_{\mathbf{b}}$ and the real roots of polynomials in $\mathcal{G}_{\mathbf{b}}$.
...
- Return $\cal{R}_\mathbf{b}$.

The algorithm for computing a monotone CAD can now be presented, making use of *MonotoneCADHelper()*.

Algorithm $\mathrm{MonotoneCad(\mathcal{D}, \mathcal{A})}$:

- Initialise $\mathcal{F}$ and $\mathcal{R}$ as above, for use with *MonotoneCadHelper*.

- Let $\mathbf{b}$ be the unique CAD cell in $\R^0$ (see Definition \@ref(def:cells)).

- Call
$$
\mathrm{MonotoneCadHelper(\mathcal{D}, \mathcal{A}, \mathbf{0})}.
$$

- Return $\mathcal{R}$.

Note that, although not returned here, refinement polynomials $\mathcal{F}$ may be useful in practice.
:::
