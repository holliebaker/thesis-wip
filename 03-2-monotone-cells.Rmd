...
We now discuss how to find the critical points of $\phi$ and $\psi$ starting with the polynomials defining cells in $\cal D$.
...
$$\mathcal{R} := \textproc{MonotoneCad}(\mathcal{D}, \mathcal{A})$$
...
Subroutine $h := \textproc{sub}_{\mathbf{y}} (g)$, where
...
\State $\mathcal{C}_0 \gets \{ \mathbf{c} \in \A \mid \dim(\mathbf{c}) = 0 \}$
\State $\mathcal{C}_1 \gets \{ C'' \subset \A \mid \dim(C'') = 1 \}$
...
\For {$C'' \in \mathcal{C}_1$}
    \For {$C \in \text{true cells of } \mathcal{D} \text{ such that} \dim(C) = 2 \text{ and } C = \projop{1}(C)$}
        ...
        \While {$j \le n$}
            ...
            \For {$C_\delta \in \{ C_B, C_T$ if not empty $\}$}
                \State $\{g_2,\ldots,g_{j}\} \gets \textproc{ZeroPolynomials}(C_\delta)$
                \State Compute
$$
...
\State $\mathbf{b} \gets \mathbf{0}$ \Comment{the unique CAD cell in $\R^0$}
