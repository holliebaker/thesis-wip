...
We now return to [@bgv15 Theorem 3.20] and discuss how to obtain monotone cells.
...
$$
forms a cylindrical decomposition of $\R^{n-k}$. This decomposition will be called the *sub-decomposition* of $\cal D$ above $c$, and $\mathbf{c}$ the *base cell* of the sub-decomposition.
:::
...
(c_1,\ldots,c_t)
$$
such that the graphs of $\varphi_{\{ c_i < x_1 < c_{i+1} \}}$ and $\psi_{\{ c_i < x_1 < c_{i+1} \}}$ for all $1 \le i \le t-1$ are monotone curve intervals. By @bgv15, Theorem 1.7, $X \cap \{ c_i < x_1 < c_{i+1} \}$ for all $1\le i \le t-1$ is a semi-monotone set [@bgv15, proof of Lemma 3.18].
...
Let $\mathcal{D}$ be a CAD of $\R^n$ compatible with each set $V_1,\ldots,V_k \subset \R^n$ such that each $2$-dimensional cell is the graph of a quasi-affine map. Let
\begin{equation}
...
Let $C$ be a $(1,0,\ldots,0,1)$-cell of the decomposition induced by $\cal D$ on $\R^{\alpha}$ such that $\projop{\alpha}^{-1}(C)$ is a $2$-dimensional cell of $\mathcal{D}$.
...
Then there is an algorithm, taking $\mathcal{D}$ and $\mathcal{A}$ as input, which produces a set of algebraic numbers $(c_1,\ldots,c_t)$ such that $C \cap \{ c_i < x_1 < c_{i+1} \}$ is monotone. Each $c_i, 1 \le i \le t$ is represented by a pair $(m_i, J_i) \in \Z[x_1] \times \Q$ (see Equation \@ref(eq:algebraic-number).
:::
...
$$
where $m_i = x_1^{d_{i,1}} \cdots x_n^{d_{i,n}}$ is a monomial in variables $x_1,\ldots,x_n$ and $a_i$ is an element of tho algebraic extension of the rational numbers $\Q(\alpha)$ for $1 \le i \le k$.
...
The correctness of the algorithm from Theorem \@ref(thm:bgv-monotone-algorithm) follows from @bgv15, Theorem 3.20 and Lemma 3.19.
...
As discussed in Section \@ref(sec:quasi-complexity), the determinant of a $(k \times k)$-matrix of polynomials with maximum degree $d$ is a polynomial of maximum dergee $kd$.
...
Thus, the degree of the polynomial defined in Equation \@ref(eq:jacobi-det) is at most $n(s(d+1))^{2^{O(n)}}$, which is asymptotically the same as $(s(d+1))^{2^{O(n)}}$.
...
A generalisation of Bezout's Theorem (see @masser1983) states that, oven an algebraically closed field, if a system of $n$ polynomial equations, having dergees $d_1,\ldots,d_n$, in $n$ unknowns has a finite number of solutions, then this number, counted with multiplicities, is at most the product $d_1 \cdots d_n$. This will be an upper bound on the number of different real solutions to the system of polynomials. Since we only have an upper bound on the maximum degree $d$ of polynomials appearing in the systems from Equation \@ref(eq:lagrange-solve), then there are at most $d^n$ distinct real solutions to the system. 
An even tighter bound can be obtained by considering the number of connected components of the algebraic set defined by the this system of equations. 
...
For a semialgebraic set $S \subset \R^n$ defined by a conjunction of any number of polynomials having maximum dergee $d$, ${\tt b}(S) \le d(2d - 1)^{n-1}$ [@gv2009, Section 6.1 (a)].
This bound is sufficient for our situation because the inequalities define a subset of these roots.
Using the upper bound on the degrees of polynomials defining the CAD and the determ0inant of the matrix, along with the bound on the number of roots given by Bezout's theorem, we get a bound for the number of refinement points of
$$
\left(s(d+1)\right)^{n \cdot 2^{O(n)}}.
$$
...
(s(d+1))^{2^{O(n)}}.
$$
Thuse, we obtain a bound on the total number of refinement points, which is asymptotically the same as the bound in the number of cells in the CAD containing quasi-affine cells. This gives us an upper bound on the complexity, number of cells, number of polynomials and dergees defining a CAD which is monotone with respect to all $V_1,\ldots,V_k$. 
