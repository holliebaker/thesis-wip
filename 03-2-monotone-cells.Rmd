...
\varphi(x,y) = x^2 + y^2.
$$
Let $Y \subset \R^3$ be the graph of function $\varphi$ on $\Delta$. Observe that $Y$ is a cylindrical cell and $\varphi\vert_\Delta$ is a quasi-affine map, since it is strictly increasing in both $x$ and $y$.
However, $y \cap \{ z = 3/4 \}$ is not connected, hence $\varphi\vert_\Delta$ is not a monotone function and $Y$ is not a monotone cell.
:::
...
We now discuss how to find these critical points of $\varphi$ and $\psi$ starting with the polynomials defining cells in $\cal D$.
...
The following helper function enables recursion on the tree-like structure of cells in $\mathcal{E}$.
...
\For {$\mathbf{c} \in \mathcal{C}_0$}
    \State Compute ${\cal A'} \gets \{ \mathcal{ A'}_{i-1} := \mathrm{sub}_{\mathbf{c}} (\mathcal{A}_i) \mid 2 \le i \le n \}$
    \State $\mathcal{D'} \gets $ sub-CAD of $\mathcal{D}$ in $\R^{n-1}$ above $\mathbf{c}$
    \State $\mathrm{MonotoneCadHelper}(\mathcal{D', A'}, \mathbf{c})$ \Comment{proceed by induction}
\EndFor
...
\For {$C'' \in \mathcal{C}_1$}
    \For {$C \in \text{true cells of } \mathcal{D} \text{ such that} \dim(C) = 2 \text{ and } C = \projop{1}(C)$}
        ...
        \While {$j \le n$}
            \State $C' := \projop{j} (C)$
            \State $C'_B \gets \rm{Bottom}(C')$ \Comment{bottom of $C'$ or $\emptyset$ if it does not exist}
            \State $C'_T \gets \rm{Top}(C')$ \Comment{top of $C'$ or $\emptyset$ if it does not exist}
            ...
            \For {$C_\delta \in \{ C_B, C_T$ if not empty $\}$}
                \State $\{g_2,\ldots,g_{j}\} \gets \rm{ZeroPolynomials}(C_\delta)$
                \State Compute
$$
...
\State $\mathcal{R}_\mathbf{b} \gets \rm{RealRoots}(\mathcal{F}_{\mathbf{b}}) \cup \rm{RealRoots}(\mathcal{G}_{\mathbf{b}})$ \Comment{compute roots}
...
\State $\mathrm{MonotoneCadHelper(\mathcal{D}, \mathcal{A}, \mathbf{0})}$ \Comment{use helper function to populate $\mathcal{F}$ and $\mathcal{R}$}
