...
The construction of monotone cells is based on the following theorem. 
...
## Two-dimensional monotone sector cells {#monotone-sectors}

The proof of Theorem \@ref(thm:bgv-monotone) proceeds in two stages: first, $X \subset \opspan{x_1,x_\alpha}$ is refined such that each $Y \subset X$ is a monotone 2-dimensional sector cell.
...
$$
\gamma := \{ (x_1,x_\alpha) \in \opspan{x_1,x_\alpha} \mid x_2 \text{ is an endpoint of an interval in } {\cal I}(x_1) \}.
$$
...
Let $C\subset \R^\alpha$ be a $(1,0,\ldots,0,1)$-cell, $X := \projops{x_1,x_\alpha} (C)$ and $X' := \projop{1}(C)$. Suppose we have the family ${\cal I}(c), c \in X'$ and set of one-dimensional curves associated with $C$. Then

1. The projection map $\projops{x_1,x_\alpha}\vert_C$ is injective. 

1. $X$ is a $2$-dimensional cylindrical sector cell.

1. For each $c \in X'$, the family ${\cal I}(c)$ contains a single open innterval.
...
(c_1,\ldots,c_t)
$$
be the critical points of functions $\varphi$ and $\psi$. Then, for all $1 \le i < t$, $\phi$ (resp $\psi$) is either strictly increasing in, strictly decreasing in, or independent of $x_1$ on the interval $(c_i, c_{i+1})$. Note that if $X$ is not bounded from below, then $\varphi$ will be a constant function (equal to $-\infty$). Likewise, if $X$ is not bounded from above, $\psi$ will be a constant function (equal to $\infty$). If this happens, the non-finite function can be ignored, since it does not havea graph. 
...
(\#eq:proj-fac-set)
\end{equation}
where ${\cal A}_i \subset \Z[x_1,\ldots,x_i]$, be the projection polynomials defining $\cal D$. For convenience, we say that $g(\mathbf{x}) = 0$ for some $g \in A_k$ and $\mathbf{x} \in \R^n$, such that $k < n$, if $g(\projop{k}(\mathbf{x})) = 0$.
...
Using $\exists_\infty$, some of the costly computations with algebraic numbers can be avoided, thereby improving efficiency in practice. **TODO refer to qepcad user documentation.**
