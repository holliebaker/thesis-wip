...
(c_1,\ldots,c_t)
$$
such that the graphs of $\varphi_{\{ c_i < x_1 < c_{i+1} \}}$ and $\varpsi_{\{ c_i < x_1 < c_{i+1} \}}$ for all $1 \le i \le t-1$ are monotone curve intervals. By @bgv15, Theorem 1.7, $X \cap \{ c_i < x_1 < c_{i+1} \}$ for all $1\le i \le t-1$ is a semi-monotone set [@bgv15, proof of Lemma 3.18].
For the graphs of $\varphi$ and $\varpsi$ to be monotone curve intervals, we require that $\varphi$ and $\varpsi$ are either strictly increasing in, strictly decreasing in or independent of $x_1$. Thus, $(c_1,\ldots,c_t)$ are the critical points of $\varphi$ and $\psi$.
...
<!--
Complexity
- refinements are easy. but how many poits do we get?
- how many 2d cells. we know how many cells in total, but how many of dimension 2?
- what is the complexity of computing the jacobi determinant. we do CAD projection to get back to R^2. so the dgree oft his polynomial is important. In practice, it was better to factorise. is there some theorem about factorising polynomials where s'd' <= d?
-->
