# A novel algorithm for the obtaining the frontier condition on a cylindrical decomposition of $\R^n$ { #sec:novel-frontier }
...
### Computing the frontier of a semialgebraic set
...
Table: (\#tab:cells-of-e-refined) Cells of the CAD $\mathcal{E}_{(0,1)}$ computed in step \ref{step:one-zero} of Section \@ref(sec:worked-example). Note that only cells which are part of $\cl{S}$ are listed.
