# Novel Frontier Condition Algorithm { #sec:novel-frontier }

So far, we have been concerned only with cylindrical algebraic decompositions which are compatible with subsets of $\R^n$ of
dimension at most $2$.
...
The following algorithm, proved by Theorem \@ref(thm:main), is proposed.
...
::: {.lemma #section-split}
Let $X \subset \R^n$ be a semialgebraic set which is semialgebraically homeomorphic to the
$d$-dimensional cube $(0, 1)^d$ and $Y \subset X$ be a semialgebraic set of dimension strictly less than $\dim(X)$.
Then $\fr{X\setminus Y} \cap X = Y$..
:::

::: {.proof}
By the definition of frontier, we have
$$\fr{X \setminus Y} \cap X =\left( \cl{X \setminus Y} \cap X \right) \setminus (X \setminus Y),$$

We need to prove that $\cl{X \setminus Y} \cap X = X$.
Since $X$ is semialgebraically homeomorphic to the $d$-dimensional open cube $(0,1)^d$, it suffices to prove that $\cl{\R^d \setminus Y} = \R^d$.
$Y$ is a semialgebraic set, so it is the union of some basic semialgebraic sets
\begin{equation}
\label{eq:basic-semialgebraic}
S_i = \{ \mathbf{x} \in \R^d \mid f_1(\mathbf{x}) = 0, \ldots, f_k(\mathbf{x}) = 0, g_1(\mathbf{x}) > 0, \ldots, g_\ell(\mathbf{x}) > 0 \}
\end{equation}
of dimension strictly less than $d$.
...
$$
\cl{\R^d \setminus Z} = \R^d \supset Z
$$
because $(\R^d \setminus Z) \subset (\R^d \setminus Y)$ implies $\cl{\R^d \setminus Z} \subset \cl{\R^d \setminus Y}$.

We prove that
$$
Z \cap \cl{\R^d \setminus Z} \supset Z
$$
and claim that every $\mathbf{x} \in Z$ is a point of $\cl{\R^d \setminus Z}$.
By the definition of closure, there is a small real number $\varepsilon > 0$ such that the open ball $B(\mathbf{x},\varepsilon)$ is contained in $\R^d \setminus Z$.
Assume for contradiction that this is false. Then $B(\mathbf{x},\varepsilon) \cap (\R^d \setminus Z) = \emptyset$.
It follows that $B(\mathbf{x},\varepsilon)$ must be contained in $Z$, but this is impossible because a set of dimension
strictly less than $d$ cannot contain a $d$-dimensional open ball.
:::
...
Since $C' \times \R$ is a union of some cells $\{C_1, \ldots, C_r\}$ of $\mathcal{D}$ and $\mathcal{D}$ satisfies the frontier condition, it follows that $\cl{C_1} \cup \ldots \cup \cl{C_r} = \cl{C_1 \cup \ldots \cup C_r} = \cl{C' \times \R}$ is a union of cells of $\mathcal{D}$.
By the cylindrical structure of $\mathcal{D}$, $\proj_{\R^{n-1}}\left(\cl{C' \times \R}\right) = \cl{C'}$ is a union of cells in $\mathcal{D'}$. Hence, $\fr{C'} = \cl{C'} \setminus C'$ is a union of cells of $\mathcal{D'}$.
...
\item \textbf{Output:}
CAD $\mathcal D$ of $\R^n$ compatible with $S$ and satisfying the frontier condition.
...
\begin{algorithmic}[1]
\State $M = (i_1,\ldots,i_{n-1}) \gets (1,\ldots,1) \in \{0,1\}^{n-1}$. % \Comment{be the initial index.}
\State $\mathcal{E}_M \gets \rm{Refine}(\mathbf{true}, \{ F \})$.
\State \Comment{Define the initial pair.}
\State $(\mathcal{E}_M, \mathcal{F}_M) = (\mathcal{E}, \emptyset)$.

\While{$(0,\ldots,0) \lex M = (i_1,\ldots,i_{n-1})$}
    \State $N \gets \rm{Decrement}(M)$.
    \State \Comment{Refine}
    \State \Comment{Refine $\mathcal{E}_M$ to be compatible with a semialgebraic set contained in the cells of $\mathcal{E}_M$ with index $\lexeq (i_1,\ldots,i_{n-1},1)$.}
    \State $\mathcal{E}_N \gets \rm{Refine}(\mathcal{E}_M, \mathcal{F}_M)$.
    \State \Comment{Compute set of frontiers.}
    \State $\mathcal{A}_0 \gets \{ F_0 \mid \text{a QFF defining all cells of $\mathcal{E}_N$ with index }(i_1,\ldots,i_{n-1},0) \}$.
    \State $\mathcal{A}_1 \gets \{ F_1 \mid \text{a QFF defining all cells of $\mathcal{E}_N$ with index }(i_1,\ldots,i_{n-1},1) \}$.
%    \State $\mathcal{A}_1 \gets \{\}$ be the family of QFFs defining all cells of $\mathcal{E}_N$ with index $(i_1,\ldots,i_{n-1},1)$.
    \State ${\mathcal F}_{N,0} \gets \left\{ \fr{F_0} \mid F_0 \in \mathcal{A}_0 \right\}$.
    \State ${\mathcal F}_{N,1} \gets \left\{ \fr{F_1 \land \neg( F_{1,B} \lor F_{1,T} )} \mid F_1 \in \mathcal{A}_1, F_{1,B}, F_{1,T} \in \mathcal{A}_0 \right\}$.
    \State \Comment{$F_{1,B},F_{1,T} \in \mathcal{A}_0$ are QFFs defining the bottom / top of cell defined by $ F_1 \in \mathcal{A}_1$.}
%    \State where $F_{1,B}$ (resp. $F_{1,T}$) is a QFF defining the bottom (resp. top) of the cell defined by $F_1$, or $\mathbf{false}$ if the bottom (resp. top) does not exist.
    \State ${\mathcal F}_N \gets {\mathcal F}_{N,0} \cup {\mathcal F}_{N,1}$.
    \State \Comment{$\mathcal{F}_N$ is contained in a union of cells of $\mathcal{E}_M$ with index $\lex(i_1,\ldots,i_{n-1},0)$.}
    \State
    \State $M \gets N$.
\EndWhile
\State \Return $\mathcal{E}_{(0,\ldots,0)}$.
\end{algorithmic}
...
the Correctness section, frontiers $\fr{S'} \cap S$ and $\fr{S'''} \cap S$ coincide with $S''$, so no further refinements of
...
\begin{subfigure}{.45\textwidth}
    \includegraphics[width=\textwidth]{gnuplot/r3-initial.png}
    \caption{Illustrates the initial decomposition constructed in the first step of the worked example (projection onto the $(x,y)$-plane).}
    \label{fig:initial-cad}
\end{subfigure}
\hfill
\begin{subfigure}{.45\textwidth}
    \includegraphics[width=\textwidth]{gnuplot/r3-initial-fr.png}
    \caption{Illustrates how $\fr{S}$ interacts with the cells of $\mathcal{E}$. The frontier is indicated by a dashed line, with the blow-up subset above the origin represented by a node. }
    \label{fig:initial-fr}
\end{subfigure}
\hfill
\begin{subfigure}{.45\textwidth}
    \includegraphics[width=\textwidth]{gnuplot/r3-refined.png}
    \caption{Shows cells of $\mathcal{E}_{(0,1)}$, the refinement of $\mathcal{E}$ compatible with $\fr{S}$ computed in
    the third.}
    \label{fig:final-cad}
\end{subfigure}
\end{figure}
...
In Notation \@ref(eq:pair), the binary index $M$ refers to the pair rather than its elements.
...
(\#eq:sequence)
\end{equation}
where $\mathbf{0} = (0,\ldots,0)$ and $\mathcal{F}_{\mathbf{0}} := \{ F \mid F \text{ is a QFF defining } C \in{\mathcal
U}_{(0, \ldots ,0)} \}$. Note that the $\lex$ notation is naturally extended to elements of the sequence defined in
Notation \@ref(eq:sequence).
...
Applying the algorithm from Proposition \@ref(prp:collins) to ${\mathcal E}_M$ and $\mathcal{F}_M$, we obtain a CAD
...
We claim that the frontiers in $C$ of the $(i_1,\ldots,i_{n-1},0)$-cells contained in $C$ is the union of the cells of $\mathcal{E}_L$ contained in $C$ with index $\lex (i_1,\ldots,i_{n-1},0)$.
...
Let $B' \subset B$ be a cell of ${\mathcal E}_L$ with index $(m_1,\ldots,m_{n-1},0) \lex (i_1,\ldots,i_{n-1},0)$, then $\dim(B') < \dim(C)$.
...
On the other hand, by the definition of $\lex$, there exists some $1 \le \ell \le n-1$ such that $m_1 = i_1, \ldots, m_{\ell-1} = i_{\ell-1}$ and $m_\ell = 0, i_\ell = 1$. This implies that $\dim(B') = m_1 + \cdots + m_{n-1} < i_1 + \cdots + m_{n-1} = \dim(C)$ as required. It follows that $\dim(B) < d = \dim(C)$ and $B$ is closed in $C$.
Since $C$ is a cylindrical cell, it is semialgebraically homeomorphic to the $d$-dimensional open cube $(0,1)^d$.
According to Lemma \ref{lem:section-split}, $\fr {C \setminus B} \cap C = B$.
Since $C \setminus B$ is a disjoint union of $(i_1,\ldots,i_{n-1},0)$-cells, $C' \subset C \setminus B$ is a connected component of $C \setminus B$.
It follows that $\fr{C'} \cap C \subset B$.
On the other hand, $\fr {C'} \setminus (\fr {C'} \cap C) = \fr{C'} \setminus C$ is a subset of $\fr {C}$, which is a union of cells of $\mathcal{E}_L$ (and of the refinement $\mathcal{E}'_L$) with index $\lex (i_1,\ldots,i_{n-1},0)$.
...
We now discuss the complexity of this algorithm.
...
Theorem \@ref(thm:main) can be extended to restricted sub-Pfaffian sets, which are defined by @gv04.
...
Care must be taken to ensure that Lemma \ref{lem:section-split} is still applicable to restricted sub-Pfaffian sets.
Every sub-Pfaffian set admits the representation as a finite union of basic sub-Pfaffian sets.
...
It is difficult to see another parameter that could make the recursion significantly shorter. Thus, if any progress is to be made towards a better asymptotic complexity, the method used may need to be based on completely different ideas.
...
Finally, since some initial CAD is refined repeatedly, the classical CAD algorithm from Proposition \@ref(prp:collins) could be replaced with an incremental algorithm, E.g., the algorithm presented by @kremer2020.
It is conjectured that substituting the CAD algorithm will reduce the average-case complexity.
However, it is worth noting that the set $X_N$ is still computed at each step, using a singly exponential algorithm, and the initial CAD may need to be refined at every step and this refinement may result in the incremental algorithm backtracking all the way to the decomposition induced on $\R^1$. This change is likely to be very useful in practice as unnecessary CAD re-computations could be avoided, e.g., if $\mathcal{E}_N$ is already compatible with the set $X_N$, the algorithm in its current form will compute the refinement even though it is not needed.
