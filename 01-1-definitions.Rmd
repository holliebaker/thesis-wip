...
We now define two concepts which will be used extensively in this thesis.
The first, of an analytic submanifold, comes from @mccallum1998 and gives us the notion of smoothness.
Informally, an analytic submanifold of dimension $k$ is a subset which "looks locally like $\R^k$". Formally, it is defined as follows:
...
We will also need the idea of a critical point of a function because we will need to find local maxima and minima of cylindrical cells, e.g., in order to onstruct quasi-affine (see Definition \@ref(def:quasi-affine)) and monotone (see Definition \@ref(def:monotone-cell)) cylindrical cells. Critical poitns of a differentiable function are defined as follows:
