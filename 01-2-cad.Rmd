...
::: { .proposition }
[@bgv15, Lemma 3.4]
Let $C \subset \R^n$ be a 2-dimensional cylindrical cell which is the graph of a quasi-affine map. Then the side-wall $W \subset \f{{C}$ has exactly two connected components, each of which is either a point or a closed curve interval. 
:::
...
Let $\lc{f} = a_d$ denote the leading coefficient of $f$ and and $\red(f} = a_{d-1}x_n^{d-1} + \cdots + a_1 x_n^1 + a_0$ be the reductum of $f$. 
...
::: { .proof }
Immediate, by applying Proposition \@ref(prp:collins-set) to $\{ F \}$.
:::
...
- for each element $f \in \prim(A)$, $f(\mathbf{x},y) = 0$ for all $y\in \R$ on a finite number of points $\mathbf{x} \in \R^{n-1}$. [@collins1998, condition WO1]

- $\proj(\mathbf{F})$ is well-oriented. [@collins1998, condition WO2]
:::
...
More recently, @mccallum2019 completed the proof that Lazard's projection operator is valid. He also proved, without the condition on well-orientedness of input polynomials, that every cell of a sign-invariant CAD constructed using Lazard's projection operator is an analytic submanifold. Thus, we can generalise Remark \@ref(rem:mc-smooth) further:
...
We will force the construction of the sign-invariant CAD and use McCallum's projection operator so as to obtain smooth cells (see Remark \@ref(rem:mc-smooth)). 
...
Brown's algorithm is implemented in QEPCAD for producing a solution formula. The algorithm has been slightly modified to produce a Tarski formula for each cell. This is done by modifying the definition of a confligting pair. The original definition of a conflicting pair is a pair of cells with the same signature and the same truth value. The definition has been modified to ignore the truth value so that all cells having the same signature are identified. With this simple modification, Brown's existing algorithm can be applied such that sufficient delineating polynomials will be available to represent a cell in the CAD in a form similar to that described in Proposition \@ref(prp:union-of-conjunctionss): a disjunction (if Brown's algorithm refines the cell) of conjunctions of polynomial equations and inequalities
