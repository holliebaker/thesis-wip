...
This algorithm will form part of the later work in constructing a CAD with monotone cells, which requires the input sets to first be partitioned into smooth manifolds (see Section \@ref(sec:quasi-affine)). However, the algorithm and implementation described in this section are useful in their own right.
...
Let $\mathbf{h} = (h_1,\ldots,h_k) \subset \Z[x_1,\ldots,x_n]$, $\mathbf{i} = (i_1,\ldots,i_k) \subset \{1,\ldots,n\}^k$, $j \in \{1,\ldots,n\} \setminus \{i_1,\ldots,i_k\}$ and $f \in \Z[x_1,\ldots,x_n]$.
...
$${\cal X} := \mathrm{Stratify}(k, F, \mathbf{h}, \mathbf{i}, G)$$

**Input:**

  - $0 \le k \le n$
  - $F = (f_1,\ldots,f_q)$, with each $f_i \in \Z[x_1,\ldots,x_n], 1 \le i \le q$ is a list of polynomials,
  - $\mathbf{h} = (h_k, \ldots,h_1, h_0)$ with each $h_i \in \Z[x_1,\ldots,x_n], 1 \le i \le k$ and $h_0 = 0$,
...
\Else
    ...
    \State $M = (\max(\deg_n(f_1), \ldots, \deg_n(f_q)), \ldots, \max(\deg_{k+1}(f_1), \ldots, \deg_{k+1}(f_q)))$
    \While {$\mathbf{m} \prec M$}
        ...
        \State $h_{k+1} \gets \partial_{(h_{1},\ldots,,h_{k}),(i_{1},\ldots,i_{k}),i_\ell}^{i_{\ell} - 1} \partial_{(h_{1},\ldots,,h_{k}),(i_{1},\ldots,i_{k}),i_\ell - 1}^{i_{\ell - 1}} \ldots\partial_{(h_{1,},\ldots,,h_{k}),(i_{1},\ldots,i_{k}),i_{k}+1}^{i_{k}+1}f_{j}$
        ...
        \State Compute $s_{k+1} \gets \partial_{(h_1,\ldots,h_k), (i_1,\ldots,i_k), i_\ell} h_{k+1}.$
        ...
        \If {$\mathrm{OpenSubset}(Y_{k+1}, U_{k+1})$}
        ...
        \Else \Comment{candidate is not smooth -- proceed by induction}
            \State $\mathcal{X}' \gets \mathrm{Stratify}(k+1, F, (h_{k+1},h_{k},\ldots,h_0), (i_\ell,i_{k},\ldots,i_0), G)$
        ...
        \If {$\rm{Empty}(X',s_{k+1})$} \Comment{no more candidate strata}
        ...
        \Else \Comment{proceed to the next lexicographical index}
            ...
            \State $F := (F,s)$
            \State $\mathbf{m} \gets \rm{NextIndex}(\mathbf{m})$.
        \EndIf
    \EndWhile
\EndIf
\EndProcedure
...
The first of these, $\mathrm{OpenSubset}$, is a subroutine which determines whether $Y_k$ is an open subset of $U_k$.
...
- Begin by calling $\mathrm{Stratify}(0, \{ f_1 := z, f_2 := x^2 - y^2 \}, (0), (0), \emptyset)$.
...
\end{align*}
      ...
      - Call $\mathrm{Stratify}(1, \{ z, x^2 - y^2 \}, (x^2 - y^2,0), (1, 0), \{ 2x \ne 0 \})$.
...
\end{align*}
      ...
      - Call $\mathrm{Stratify}(1, \{ z, x^2 - y^2, 2x \}, (2x,0), (1, 0), \emptyset)$.
...
The property that this determinant is not equal to zero at every point in $Y$ ensures that $Y$ is an analytic
submanifold of codimension $k$ (see Definition \@ref(def:analytic-submanifold)). I.e., in the neighbourhood of each point $\mathbf{x}\in Y$, $Y$ is the intersection of $k$ coordinate hyperplanes. In fact, these hyperplanes are the algebraic sets $\{h_j = 0\}$ for $1\le j \le k$ because $h_1,\ldots,h_k$ are chosen such that they are algebraically independent. Note that the set $\{ h_1 = 0,\ldots,h_k = 0 \}$ itself may be singular. It could even consist of several connected components having different dimensions. However, the property holds at every point in the input set $X$.
...
$$
where $\ast_i, 1 \le i \le s \in \{<, >, =\}$. Observe that there are $s^3$ sign sets.
...
Full source code can be found here: https://github.com/monotone-cad/smooth-stratification.
...
such that $X = \{ f_1 = 0, \ldots, f_q = 0, g_1 > 0, \ldots, g_r = 0 \}$ is the basic semialgebraic set to be stratified.
...
- `Word Ineqs`: *prenex* (`QEPCAD-B`) formula of inequalities $\{ g_1 > 0, \ldots, g_\ell >0 \}$,
...
- `Word Ineqs`: list of atomic `QEPCAD-B` formulas $\{ g_1 > 0 \},\ldots,\{ g_\ell > 0 \}$
...
The function constructs $F$, the defining formula for $S$ and uses `QEPCAD-B` to construct a CAD of $\R^r$ compatible with $S$. If $S$ is empty, then this CAD will contain no true cells, so the function checks this condition.
