...
Lojasiewicz extended this result by proving that every real semianalytic set admits a smooth stratification [@lojasiewicz1964, @lojasiewicz1964]. However, the proof was non-constructive and did not provide any information on the class of functions defining the strata. 
...
::: {.definition}
[@gv04, Definition 6.2]
A finite basic weak stratification of a semi-pfaffian set $X \subset \R^n$ is a partition of $X$ into a finite number of non-singular manifolds $X_k, 0 \le k \le n$ where each $X_k$ is effectively nonsingular and has codimension $k$.
:::
...
We can formulate this question as a quantifier elimination problem. $Y_k$ is an open subset of $U_k$ if
$$
\forall \mathbf{x} \in Y_k \forall \varepsilon > 0 \> \exists \mathbf{y} \in Y_k \mid \mathbf{y} \in U_k, \Vert \mathbf{x} - \mathbf{y} \Vert < \varepsilon
$$
is true. Here we use the definition of open set using open balls. We must intersect the open ball with $U_k$ because $Y_k$ may be open in $U_k$ but not in $\R^n$. 
...
$$
U_k \cap \{ x_i = \tt{const} \mid i < i_1 \}.
$$
This reduces to the emptiness check again.
...
$$
This problem can be decided using the singly-exponential quantifier elimination algorithm from @bpr2006, Algorithm 14.21. However, this algorithm is theoretical and, in practice, we can use quantifier elimination by CAD to decide whether this set is empty. 
...
### From Elementary Algebraic sets to generic Semialgebraic sets {#sec:strat-semialgebraic}
