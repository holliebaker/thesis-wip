...
The algorithm described by @gv95 makes use of an oracle -- a hypothetical black-box function which performs a given computation in one step -- to determine whether a semi-Pfaffian set is empty.
...
\Else
    ...
    \State $X' \gets X = \{ f_1 = 0,\ldots,f_q = 0, g_1 > 0, \ldots, g_r > 0 \}$,
    ...
    \State $\mathbf{m} = (i_n,\ldots,i_{k+1},j) \gets (0,\ldots,0,1) \in \Z_{\ge 0}^{n-k+1}$
    \While {$\mathbf{m} \prec \rm{MaxIndex}$}
        \State $(0,\ldots,0,i_\ell,\ldots,i_{k+1},j) = \mathbf{m}$
        \State $\mathbf{j} \gets (0, \ldots, 0, i_{\ell} - 1, i_{\ell - 1}, \ldots, i_{k+1},j).$
        \State Let $s_{k+1} \gets \partial_{(h_{1,},\ldots,,h_{k}),(i_{1},\ldots,i_{k}),\ell}^{i_{\ell}}\ldots\partial_{(h_{1,},\ldots,,h_{k}),(i_{1},\ldots,i_{k}),i_{k}+1}^{i_{i}+1}f_{j}$ 
        \State \Comment{where $f_j$ is the polynomial in $\cal F$ with index $(0,\ldots,0,j)$}
        \State \Comment {$h_{k+1}$ is a polynomial in $\cal F$: can be found by index.}
        \State Compute $s_{k+1} \gets \partial_{(h_1,\ldots,h_k), (i_1,\ldots,i_k), i_\ell} h_{k+1}.$
        \State
        \State Define the sets $Y_{k+1}$ and $U_{k+1}$ as follows:
        ...
        \If {$Y_{k+1}$ is an open subset of $U_{k+1}$}
            \State $X_{k+1} \gets X_{k+1} \cup Y$
            \Comment{smooth and has codimension $k+1$}
        \Else \Comment{proceed by induction}
            \State Compute $(X^{\mathbf{m}}_{k+2},\ldots,X^{\mathbf{m}}_n) \gets {\rm Stratify}(k+1, F, (s,\mathbf{h}), (i_\ell,\mathbf{i}), G)$
            \If {$\rm{Empty}(X,s)$}
                \State \Return ${\cal X}_{k+1} = (X_{k+1}, X_{k+1}, \ldots, X_n)$ 
            ...
            \Else \Comment{proceed to the next lexicographical index}
                ...
                \State ${\cal F} := {\cal F} \cup \{ ((0,\ldots,0,i_\ell,\ldots,i_{k+1},j), s) \}$,
                \State $F := (F,s)$
...
In practice, quantifier elimination can be performed using CAD. This, of course, has complexity doubly exponential in
...
This algorithm has been implemented in C, using the computer algebra library `SACLIB`. `QEPCAD-B` is used to perform emptiness checks using cylindrical algebraic decomposition. Below is an example of the program's output, given the basic semialgebraic set
$$
...
`QEPCAD-B` "prenex" formula). The stratification is then computed and there is a basic REPL which allows the user to see information about the polynomials and strata produced.
...
- `Word r` $\in \N$,
- `Word V`: variable list, `Word Ineqs`: *prenex* (`QEPCAD-B`) formula of inequalities $\{ g_1 > 0, \ldots, g_\ell >0 \}$,
