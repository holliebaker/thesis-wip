...
Note that this definition does not assure any other desirable properties of the manifolds $X_k$. E.g., the closure of a stratum may not be a union of strata.
...
There are $s^{n} (d + 1)^{1^{O(n)}}$ candidate strata $Y_k$, and the algorithm determines whether each of them is empty, in order to determine whether all partial differential equations vanish identically on the input set $X$.
...
$$
\left(s (d+1)\right)^{2\cdot 2^{O(n)}} = \left(s (d+1)\right)^{2^{O(n)}},
$$
The total complexity of stratification is therefore
$$
\left(s (d+1)\right)^{2\cdot 2^{O(n)}} = \left(s (d+1)\right)^{2^{O(n)}},
+ s^{n+1} (d + 1)^{1^{O(n)}},
$$
