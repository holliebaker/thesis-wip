...
I wish to thank my supervisor, Nicolai Vorobjov, for all his help, guidance and advice throughout my studies and for encouraging me to pursue a PhD.
I would also like to thank James H. Davenport for his help and for being my lead examiner at my PhD Confirmation where he provided excellent guidance and feedback.
Thanks also to Thomas Powell, my second Confirmation examiner, for his great advice and feedback, and for providing a perspective from a different area of theoretical computer science.
...
In addition, I would like to thank Matthew England, Tereso del Rio Almajano and the "Pushing Back the Doubly Exponential Wall" research group at the Universities of Bath and Coventry for insightful informal discussions in relation to the novel algorithm for computing a CAD with frontier condition.
This PhD research was funded by an EPSRC DTP grant, without which I would not have been able to complete my studies.
Finally, I would like to thank my viva examiners, Gregory Sankaran and Matthew England for a thorough, constructive and thoughtful
examination of my work, and for all their support throughout the corrections process.
...
@lazard1979 also presented an algorithm, based on Macaulay's multivariate resultants, for solving systems of homogeneous polynomial equations over any field (algebraically closed or otherwise), which has complexity singly exponential in the number of unknowns.
...
Tarski's paper can be obtained online here: \url{https://doi.org/10.1007/978-3-7091-9459-1\_3} and @vdd1988tarski
discusses Tarski's Theorem further.
...
on classical CAD and simple tools such as refinements by linear equations and splitting $2$-dimensional cells by
intersecting them with a given curve interval.
...
detail. However, methods to obtain these properties are not given. The bulk of this thesis is concerned with transforming this constructive proof into an algorithm, presented as pseudo-code, and then implementing it. We wil also perform complexity analysis.
...
[@kleene1952, $\S 57$]). This means that the complexity can be expressed as a tower of exponents of finite height.
...
We formulate this construction as pseudo-code and specialise it to the case of semialgebraic sets where it is possible
to implement the subroutine for checking whether a candidate stratum is empty.
This algorithm has been implemented in C on top of `SACLIB`. `SACLIB` is an open-source C/C++ library for manipulating
real polynomials and real algebraic numbers and working with their solutions as real algebraic numbers (see @saclib).
The source code is hosted on
Zenodo.^[[https://zenodo.org/records/14944918](https://zenodo.org/records/14944918)]
...
The main goal of Chapters \@ref(sec:quasi-affine), \@ref(sec:monotone-cells) and \@ref(sec:frontier-bgv) is to convert
the constructive proof from @bgv15 into an algorithm by explaining how the required constructions can be achieved. Thee
algorithm is presented as pseudo-code and we obtain complexity upper bounds for the algorithm. This turns the proof that
such a decomposition exists into a sequence of steps which can be run on a computer.
...
We present an algorithm wich takes a family $V = (V_1,\ldots,V_k)$ of bounded semialgebraic sets, each of which has
dimension at most two as input and constructs a CAD compatible with each $V_i, 1\le i \le k$. Each cell $C \subset V$ of
this decomposition is smooth, i.e., if it has dimension $k$, then it looks locally like $\R^k$ (see Definition
\@ref(def:analytic-submanifold)) and includes in its definition $k$ defining polynomials (see Definition
\@ref(def:eff-non-sing)).
...
We observe that the smooth locus can be found using the smooth stratification algorithm from Chapter
\@ref(sec:smooth-strat). The smooth stratification algorithm also gives us, for each cylindrical cell of codimension
$k$, a set of polynomial equations $h_1 = 0,\ldots,h_k = 0$ which define an algebraic set which has codimension $k$ on
the cell in question. This will be important in later steps.
Next, starting with the relationship between the ranks of Jacobi matrices and critical points, we compute some new polynomials, based on the projection polynomials defining each $1$- and $2$-dimensional cell, which are equal to zero at the required critical points of projections.
...
Each stage of the algorithm has been presented as pseudo-code, along with a complexity upper bound, and Chapter
\@ref(sec:implementation-and-testing) discusses its implementation on top of Brown's `QEPCAD-B` [@brownQepcad] and
presents some illustrative examples of the software at work. The source code is available on Zenodo.^[[https://zenodo.org/records/15095148](https://zenodo.org/records/15095148)]
