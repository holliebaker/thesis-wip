...
with each $g_i \in \Q[x_1,x_2]$, and $\mathbf{p} = (p_1,p_2)$ be a common root of $g_d,\ldots,g_1,g_0$ (the coefficients of the powers of $x_3$).
...
::: {.proof}
Since $f$ is irreducible, it has at least two coefficients in $\Q[x_1,x_2]$ and their GCD is a constant. Let $g_i,g_j \in \Q[x_1,x_x]$ be two of these coefficients.
...
::: {.remark}
Let $C$ be an $(i_1,\ldots,i_{n-1},0)$-cell as described in Lemma \@ref(lem:ext-lazard-5-13) such that $i_1=i_k = 1$ for
...
Let $g_{i_1},\ldots,g_{i_{n-2}}\in \Q[x_1,\ldots,x_{n-1}]$ be irreducible, pairwise relatively prime polynomials having $\mathbf{p}$ as a root.
Then the limits of the common roots of $f$ and $g_{i_1},\ldots,g_{i_{n-2}}$ with $\{i_1,\ldots,i_{n-2}\} =
...
$g_{i_1},\ldots,g_{i_{n-2}}$ are irreducible and pairwise relatively prime, the ideal generated by the coefficients of $f$ and
...
Pseudo-code appears on the next page.
\newpage
\begin{algorithmic}[1]
\Procedure{LazardLift3}{$\mathcal{A}, \mathcal{D}$}
\State ${\cal R} \gets \emptyset$
...
\For{each $(i_1,\ldots,i_{k-1},0)$-cell $E$ in $\cal C$}
  ...
  \State $\mathcal{A'}_{j} := \{ {\textproc{sub}}_{\mathbf{c}} (f) \in \Z[x_{\ell_1},\ldots,x_j] \mid f \in \mathcal{A}_{j
  - \ell_1 + 1} \}$
  \State \Comment{set of polynomilas defining the sub-CAD above $\mathbf{c}$}
  \State $\{h_{j_1},\ldots,h_{j_{k-\ell_1 + 1}}\} :=
  \textproc{DefiningPolynomials}(C,\mathcal{A})$
  \State \Comment{defining polynomials for $C$}
  \State \Comment{determine whether it is a bad cell}
  \For {$\mathbf{b} \in \fr{\projop{k-1}(C)}, \dim(\mathbf{b}) = 0$}
  \If {$\textproc{IsBad}(\mathcal{D},\mathbf{b})$}
    \State $g_{\ell_11} \in {\cal A}_{\ell_1}, g_{\ell_2} \in {\cal A}_{\ell_2}$ such that $g_{\ell_1}(\mathbf{b}) = 0$
    and $g_{\ell_2}(\mathbf{b}) = 0$
    \State ${\cal A'} := {\cal A}_1 \cup {\cal A}_2$
    \State \Comment{compute saturations}
    \State $I_1 \gets \langle f, g_{j_1}, \ldots, g_{j_{k - \ell_1 + 1}}, g_{\ell_2}, 1 - z g_{\ell_1} \rangle \cap \Z[x_1,\ldots,x_k]$
    \State $I_2 \gets \langle f, g_{j_1}, \ldots, g_{j_{k - \ell_1 + 1}}, g_{\ell_1}, 1 - z g_{\ell_2} \rangle \cap
    \Z[x_1,\ldots,x_k]$
    \State \Comment{where $z$ is a new variable}
    \State \Comment{saturation of ideal by $h$}
    \State Let $S_1, S_2$ be generator systems for the ideals $I_1$ and $I_2$
    \State $L \gets \{ {\textproc{sub}}_{\mathbf{b}}(f') \in \Q[x_3] \mid f' \in S_1 \cup S_2 \}$
    \State compute real roots $(c_1,\ldots,c_t)$ of $L$
    \State Let ${\cal R} := {\cal R} \cup \{ {\cal R}_{\mathbf{b}} = (c_1,\ldots,c_t) \}$
  \EndIf
  \EndFor
