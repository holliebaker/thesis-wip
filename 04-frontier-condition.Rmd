...
(sd)^{O(n^2)},
$$
where $C$ is defined by $s$ different polynomials of maximum dergee $d$.
$C$ has constant sign on all polynomials defining the CAD (although it may be the case that only some of them will be needed to define it), so, using the bounds discussed in Sections \@ref(sec:quasi-complexity) and \@ref(sec:monotone-complexity), the complexity upper bound for computing $\fr{C}$ is
$$
\left( \left( s (d+1) \right)^{2^{O(n)}} \left( s (d+1) \right)^{2^{O(n)}} \right)^{O(n^2)}
= \left( s (d+1) \right)^{O(n^2) \cdot 2\cdot 2^{O(n)}}
,
$$
...
In order to do this, a similar process as that used to obtain monotone cells (see Section \@ref(sec:monotone-cells)) can be used. This will be repeated for each $2$-dimensional cells contained in $V_1\cup \ldots \cup V_k$. As such, the overall complexity will be the same as that presented in Section \@ref(sec:monotone-complexity). I.e.,
$$
(s(d+1))^{2^{O(n)}}.
$$
...
Both approaches -- @bgv15 and the gerenalisation of @lazard10 -- have the same complexity upper bound.

To sum up, each of the three stages -- quasi-affine, monotone and frontier condition -- of the algorithm for computing a CAD which is monotone with respect to all $V_1,\ldots,V_k$, such that $\dim(V_i) \le 2$ for all $1 \le i \le k$, and satisfying the frontier condition each has complexity $(s(d+1))^{2^{O(n-1)}}$.
...
(sd)^{O(1)^n}.
$$
Therefore, the monotone CAD algorithm has, as expected, a higher complexity than that of the classical algorithm.
However, the complexity is not significantly worse, still being doubly exponential in the number of variables and can still (very loosely) be said to have complexity $2^{2^n}$.
