...
The above, as Lemma \@ref(lem:frontier), is proved, proved and discussed in more detail in Section \@ref(sec:novel-frontier-fr).
...
\mathbf{b}$ and $f(\mathbf{x}) = 0 \forall \mathbf{x} \in C$
\EndProcedure
...
\For{each $(0,0)$-cell $\mathbf{b}$ in $\cal D'$}
  ...
  \If {$\mathrm{IsBad}(\mathcal{D},\mathbf{b})$}
    ...
    \For {each $g \in {\cal A'}$}
      ...
      \State $L \gets \{ {\mathrm{sub}}_{\mathbf{b}}(f') \in \Q[x_3] \mid f' \in S \}$
