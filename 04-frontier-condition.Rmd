...
- refine the $(0,1,1)$-cell $\{ x = 1 \}$, which is actually a sub-CAD of $\R^2$ above $x=1$ to be compatible with the
...
First suppose that $C$ is a section cell. I.e., it is a root of some polynomial $f$ and $\fr{C}$ is defined as the limit
of this root over the boundary of $C'$. Thus $\projop{n-1}$ is a continuous, injective map from $\fr{C}$ to $\fr{C'}$.
This follows from the condition that the decomposition induced by $\cal D$ on $\R^{n-1}$ is strong, and the property that if the root defining the section $C$ is infinite over some cell $C'' \subset \fr{C'}$, then it is also infinite over $\fr{C''}$.
...
$\{ g = 0, h = 0 \} \subset \R^2$ is zero-dimensional and contains $\mathbf{p}$.
...
We first consider Proposition \@ref(prp:lazard-5-3) (@lazard10, Theorem 5.3), which states that any cell in $\R^3$
which projects on a topologically regular, closure finite cell, is well-bordered. This is essential to Lazard's method,
since it relies on blow-up points being zero-dimensional. @lazard10, Theorem 5.3 is presented below:
...
and $C_T$ which project onto $C'$ the side-walls $W = \fr{C} \setminus (C_B \cup C_T)$ which consist of points or intervals projecting onto the endpoints of $C'$.
...
We conjecturee that this result can be extended to cells of dimension $\le 2$ in a sign-invariant CAD of $\R^n$, as long
as the CAD is strong about the projection of the cell. In $\R^n, n > 3$, blow-ups may have dimension greater than $0$.
However, we claim that is a blow-up of dimension $1$ occurs in the frontier of the projection of a cell, the cell tends
to $-\infty$ or $\infty$ as it approaches the blow-up.

::: {.conjecture #ext-lazard-5-3 }
(extension to @lazard10, Theorem 5.3)
...
This follows from the fact that the Zariski closure of $C$ is an algebraic subvariety of dimension $2$ so it cannot
contain a proper subvariety which has dimension $2$.
If the polynomial which is equal to $0$ at all points of $C$ vanishes identically in a $2$-dimensional cell contained in
$\fr{C'}\times \R$, then $C$ tends to $-\infty$ and $\infty$ as it approaches this cell.

The $(i_1,\ldots,i_{n-1},1)$-cells of $\mathcal{D}$ contained in $C' \times \R$ also have the same arrangement as
$(1,1,1)$ cells from Proposition \@ref(prp:lazard-5-3) (@lazard10, Theorem 5.3). Being careful to take blow-up points into account, the same argument as in the proof of Proposition \@ref(prp:lazard-5-3) (@lazard10, Theorem 5.3) shows that these cells are also well-bordered.

Conjecture \@ref(cnj:ext-lazard-5-3) always holds if the frontier of the projection of the cell in question contains
only $0$-dimensional blow-ups. If $1$-dimensional blow-ups exist above which the frontier of the cell should contain an
open interval, the conjecture fails. However, the algorithm due to @bgv15, presented in Section \@ref(sec:frontier-bgv)
will still work. Since it is easy to check the dimension of a blow-up by checking the dimension of the cell and the
signs of projection polynomials on it, we can easily revert to the approach from @bgv15 if a situation arises where
Conjecture \@ref(cnj:ext-lazard-5-3) fails.

Now let us suppose the conjecture is true.
...
This implies that, if Conjecture \@ref(cnj:ext-lazard-5-3) holds, we should be able to use a similar technique to that described in Proposition \@ref(prp:lazard-5-14)
...
Let us begin with the situation described in Proposition \@ref(prp:lazard-5-14) to see how it can be extended.
...
The cell $C$ is topologically regular since the projection of a monotone cell is monotone and $C$ is well-bordered by
Conjecture \@ref(cnj:ext-lazard-5-3) since $\dim(C) \le 2$.
...
\For{each $(i_1,\ldots,i_{k-1},0)$-cell $E$ in $\cal C$}
  ...
  \State \Comment{determine whether it is a bad cell}
  \For {$E' \in \fr{\projop{k-1}(C)}$}
  \If {$\textproc{IsBad}(\mathcal{D},E')$}
    \If {$\dim(E') > 0$} \Comment{high-dimensional blow-up}
        \State fall back to @bgv15 or fail gracefully
    \EndIf
    \State $\mathbf{b} \gets E'$ \Comment {only $0$-dimensional blow-ups allowed}
    \State $\mathcal{R}_{\mathbf{b}} \gets \emptyset$ \Comment{initialise set of refinement points above $\mathbf{b}$}
    ...
    \For {each $f_{\ell_1} \in \mathcal{A}_{\ell_1},f_{\ell_2} \in \mathcal{A}_{\ell_2}$}
      ...
      \State $I_1 \gets \langle h_k,\ldots,h_{\ell_2 + 1}, g_{\ell_2}, h_{\ell_2 - 1}, \ldots, h_{\ell_1 + 1}, 1 - z
      f_{\ell_1} \rangle \cap R$
      \State $I_2 \gets \langle h_k,\ldots,h_{\ell_2 + 1}, h_{\ell_2 - 1}, \ldots, h_{\ell_1 + 1}, g_{\ell_1}, 1 - z
      f_{\ell_2} \rangle \cap R$
      \State \Comment{where $z$ is a new variable and $R = \Z[x_1,\ldots,x_k]$}
      \State \Comment{saturation of ideal by $h$}
...
decomposition induced by $\mathcal{D}$ on $\R^{k-1}$ is strong about $C$. Conjecture \@ref(cnj:ext-lazard-5-3) asserts
