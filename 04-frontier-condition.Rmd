...
Now suppose we have a CAD $\cal D'$ monotone with respect to $V_1,\ldots,V_k$. Our final step, according to [@bgv15, Theorem 3.10] is to refine the frountier $\partial C$ of each $2$-dimensional cell $C$ such that $C \subset V$ so that $\partial C$ is a union of cells of $\cal D'$.
...
Let $\mathbf{c} = (c_1,\ldots,c_n)$ be one of the one-dimensional components of $\cal U$. If $c_1$ is not a $0$-dimensional cell of the decomposition induced by $\cal D'$ on $\R^1$, then perform a refinement of the kind $x_1 < c_1, x_1 = c_1, x_1 > c_1$. Recall that these refinements, according to [@bgv15, Lemma 3,11], preserve the cylindrical structure and monotone cells. Otherwise, $C_1$ is a $0$-cell. Proceed by induction on the sub-cad of $\cal D'$ above $c_1$.

Now let $T \in {\cal U}$ be one of the one-dimensional components. If $T$ is not an existing 1-dimensional cell of $\cal D'$, then it is contained in a 2-dimensional cell, $Z$, of $\cal D'$. In fact, it divides $Z$ into two cells which, by [@bgv13, Theorem 11], $Z \setminus T$ consists of two 2-dimensional monotone cylindrical cells. Hence, replacing $Z$ by $T$ and the two components of $Z \setminus T$ makes $Z$ compatible with $\partial C$ and ensures that $Z$ is refined into monotone cells. 

Repeating the process for each $0$- and $1$-dimensional components $\mathbf{c}$ of $\cal U$, we obtain a refinement of $\cal D'$ compatible with $\partial C$. 
This completes the construction of a monotone CAD due to @bgv15. 

The drawback in translating this construction into an algorithm is that we need to compute $\partial C$ for each 2-dimensional cell $C \subset V$. 
...
@lazard2010 presents an algorithm for constructing an $\mathbf{F}$-invariant cylindrical algebraic decomposition of $\R^n, n\le 3$ such that every cell is a topologically regular set and the frontier condition is satisfied. 

According to [@lazard10, TODO], in dimension $\le 2$, there is nothing to do. 
Let $\cal D$ be the CAD of $\R^3$ constructed by @lazard10 at the end of Section 5,2. The results in [@lazard10, Section 5.3] assume two properties of $\cal D$. Firstly $\fr(C)$, where $\dim(C) = k$, is the closure of the union of some cells of $\cal D$ of dimension $k-1$. Secondly, ([@lazard10, Proposition 5.13]), any bad cells in the decomposition induced by $\cal D$ on $\R^2$ are 0-dimensional isolated points.

::: {.lemma} [@lazard10 Proposition 5.13]
...
(E.g., in the decomposition of the whitney umbrella, refined such that each cell is topologically regular, let $C$ be $\{ x>0, y>0, x^2 - y^2 z = 0 \}$, then case 2 applies. Let $D = \{ x = 0, y = 0 \}$ and $E = \{ x > 0, y = 0, z = 0 \}$. The first case applies if we choose $C = D$. The same case 2 construction applies if we choose $C$ to be a sector (either 3 dimensional above or below $\{ x > 0, y > 0, x^2 - y^2 z = 0 \}$ or 2-dimensional above or below $\{x > 0, y = 0, z = 0 \}$).)
...
- $(1,1,0)$-cell: these are necesarily the root of a polynomial $f \in \Q[x_1,x_2,x_3]$ which vanishes identically on $D'$. 
Let $V_r$ be a small neighbourhood (with radius $r > 0$) centered aronud $D'$ and consider $V_r'$, the cylinder above the boundary of $V$. Since $C$ is a monotone cell, the maximum and minimum values of $x_3$ in this intersection are reached on the boundary of $C$, for sufficiently small $r$. The limits, as $r$ tends to $0$, of this maximum and minimum clearly belong to the boundary of $B$, if these limits do not tend to infinity. Note that both the maximum and minimum cannot be infinite, since this would imply that either $B$ is empty, or coincides with $D$. Therefore, the non-infinite limits are the endpointns of some $1$-dimensional cell belonging to the boundary of $C$. Case 2 applies, taking $E$ to be this $1$-dimensional cell$. 

- $(0,0,1)$- or $(0,0,0)$-$cells: cannot satisfy the hypothesis. 
:::
...
Let $g\in \Q[x_1,x_2]$ be an irreducible polynomial having $p$ as a root. 
The limits of the common roots of $f$ and $g$ as $(x_1, x_2) \to p, (x_1, x_2) \ne p$ may be computed as solutions of a zero-dimensional
polynomial system.
:::
...
It follows that the ideal generated by $S$ and the coefficients $g_0,\ldots,g_d$ of $f$ is zero-dimensional and the limits of $\{ f=0,g=0 \}$ as $(x_1,x_2) \to p$ can be computed as the zeros of $S$. 
:::

### An aside on polynomial ideals and Zariski closures

@lazard10 uses the trick of saturating the ideal $\langle f,g \rangle$ by some $h$, which is not a multiple of $g$. The key to this trick lies in Zariski closures. Let us take an aside to discuss this. We closely follow @cox2013 and all results are proved in a general polynomial ring $K$. We will discuss whether the results can be applied over the reals.

#### Ideals, Groebner bases and Buchberger's algorithm

::: {,definition}
Let $K[x_1,\ldots,x_n]$ be a polynomial ring. A subset $I \subset K[x_1,\ldots,x_n]$ is called an {\em ideal} if
...
Some examples of monomial orderings used in constructing Groebner bases include
...
$$
where $\lt(I) = \{ \lt(f) \mid f \in I \}$
:::

::: {.definition}
We write $\overline{f}^{F}$ for the remainder of division of $f$ by the tuple $F = (g_1,\ldots,g_t)$. If $F$ is a Gr\"obner basis, $F$ can be considered as an (unordered) set by [@cox2013 p83, Proposition 1].
:::

::: {.definition}
Let $f,g \in K[x_1,\ldots,x_n]$ such that $\multideg(f) = \alpha$ and $\multideg(g) = \beta$. Define $\gamma = \max(\alpha, \beta)$ (where $\max$ is taken component-wise). $x_1^{\gamma^1} \cdots x_n^{\gamma^n} = \lcm ( \lm(f), \lm(g) )$ is called the {\em least common multiple} of the leading monomials of $f$ and $g$. 
:::
...
Now consider $(ax^\alpha + f') + (bx^\beta + g') = (ax^\alpha + bx^\beta) + (f' + g')$. We have $\multideg(f') < \alpha$ and $\multideg(g') < \beta$ so it suffices to consider $ax^\alpha + bx^\beta$. If $\alpha \ne \beta$ then $\multideg(f+g) = \max(\alpha, \beta)$. If $\alpha = \beta$ then if $a+b\ne 0$ we have $ax^\alpha + bx^\alpha = (a+b)^\alpha$, hence $\multideg(f+g) = \alpha$. Otherwise we have $ax^\alpha + bx^\alpha = 0x^\alpha$ so $\multideg(f + g) = \multideg(f' + g') < \alpha$. This proves property 2.
:::

::: {.lemma #lem:remainder-condition}
Let $I \subset K[x_1,\ldots,x_n]$ be an ideal, $G = \{g_1,\ldots, g_t\}$ be a Gr\"obner basis for $I$ and $f$ be an arbitrary polynomial in $K[x_1,\ldots,x_n]$. 
Then there exists a unique $r \in K[x_1,\ldots,x_n]$ such that

1. no term of $r$ is divisible by any of $\lt(g_1), \ldots, \lt(g_t)$, and
...
Now, to prove that $r$ is unique, suppose that $f = g + r = g' + r'$ for $g,g'$ and $r,r'$ satisfying both conditions. Then $r-r' = g-g' \in I$ (by definition of an ideal). Assume $r\ne r'$, then $\lt(r-r') = \langle \lt(I)\rangle = \langle \lt(g_1),\ldots, \lt(g_t) \rangle$ since $G$ is a Gr\"obner basis for $I$. By [@cox2013 p70, Lemma 2], this implies $\lt(r-r')$ is divisible by some $\lt(g_i)$, which is a contradiction since $r$ and $r'$ cannot be divisible by any of the $\lt(g_i)$s. 

The final part of the proposition follows from uniqueness of $r$. 
:::

::: {.proof cor:div-zero}
Let $G = \{ g_1,\ldots,g_k \}$ be a Gr\"obner basis for an ideal $I \subset K[x_1,\ldots,x_n]$ and let $f \in K[x_1,\ldots,x_n]$. Then $f \in I$ if and only if the remainder on division of $f$ by $G$ is zero. 
:::
...
$$
S(f,g) = \dfrac{\lcm(\lt(f),\lt(g))f}{\lt(f)} - \dfrac{\lcm(\lt(f),\lt(g))g}{\lt(g)}.
$$
:::
...
If $\multideg( \sum_{i=1}^s p_i )$ is a linear combination $\sum_{(i,j) \in \{1\ldots,s\}^2} a_{ij} S(p_i,p_j)$ where all $a_{ij} \in K$.  
Furthermore all $\multideg(S(p_i,p_j)) < \delta$. 
:::
...
In order for $\multideg(\sum_{i=1}^s p_i) < \delta$, leading monomials of $p_i$'s must cancel in the sum. I.e., $\sum_{i=1}^s c_i\lt(p_i) = 0$ implies $\sum_{i=1}^s c_i = 0$. 

Since $\multideg(p_i) = \multideg(p_j)$ for all $i.j$, $\lcm(\lt(p_i),\lt(p_j)) = x_1^{\delta_i} \cdots x_n^{\delta_n} = h = \tfrac{1}{c_i}\lt(p_i) = \tfrac{1}{c_j}\lt(p_j)$. Consider
$$
S(p_i,p_j) = \dfrac{\lcm(p_i,p_j)p_i}{\lt(p_i)} - \dfrac{\lcm(p_i,p_j)p_j}{\lt(p_j)} = \dfrac{h p_i}{c_i h} - \dfrac{h p_j}{c_j h} = \dfrac{1}{c_i}p_i - \dfrac{1}{c_j}p_j.
$$
Hence
$$
...
::: {.proof}
$\Rightarrow:$
If $G$ is a Gr\"obner basis for $I$, then $S(g_i,g_j) \in I$ for all $g_i,g_j \in G$ and, by Corollary~\ref{cor:div-zero}, the remainder of $\overline{S(g_i,g_j)}^F$ is zero. 

$\Leftarrow:$
Let $f \in I$ be a nonzero polynomial. We need to show that $\lt(f) \in \langle \lt(g_1), \ldots, \lt(g_k) \rangle$. We have to show that
$$
f = \sum^k_i=1 h_ig_i
$$
...
$$
is minimal. Such a $\delta$ exists by the well-ordering property. We have
$$
\multideg(f) \le \delta = \max(\multideg(h_ig_i) \mid h_ig_i \ne 0)
$$

First consider the case when $\multideg(f) = \delta$. We have $\multideg(f) = \multideg(h_ig_i)$ for some $1 \le i \le k$. Therefore $\lt(f)$ is divisible by $\lt(g_i)$. Hence $\lt(f) \in \langle g_i, \ldots, g_k \rangle$.

Now suppose that $\multideg(f) < \delta$. We can write $f$ as follows
$$
\sum_{i=1}^{k}h_{i}g_{i}=\sum_{i=1}^{k}\lt(h_{i})g_{i}+\sum_{i=1}^{k}(h_{i}-\lt(h_{i}))g_{i}
$$
and use this representation to isolate terms with maximum multidegree
$$
\sum_{\multideg(h_{i}g_{i})=\delta}\lt(h_{i})g_{i}+\sum_{\multideg(h_{i}g_{i})=\delta}(h_{i}-\lt(h_{i}))g_{i}+\sum_{\multideg(h_{i}g_{i})<\delta}h_{i}g_{i}.
$$
The last two terms have $\multideg < \delta$, so we attempt to rewrite $\sum_{\multideg(h_ig_i)= \delta} \lt(h_i)g_i$ to decrease $\delta$, thereby obtaining a contradiction. Applying Lemma~\ref{lem:linear-combination} with $p_i = \lt(h_i)g_i$ with multidegree $\delta$ and $f$ with multidegree $< \delta$, we can write $\sum_{\multideg(h_ig_i) = \delta} \lt(h_i)g_i$ as a linear combination (with coefficients in $K$) of S-polynomials having multidegree $< \delta$. Hence, we know that $f$ can be rewritten as a sum of polynomials with multidegree $< \delta$, contradicting the assumption that $\delta$ is minimal.
...
\end{align}
The second and third sums in Equaton \@ref(eq:multideg-delta-simp) have multidegree $< \delta$. Since $\multidegree(f) < \delta$, the multidegree of the first sum in Equation \@ref(eq:multideg-delta-simp) must also have multidefree $< \delta$.
...
$$
S(\rm{lt}h_i g_i, \rm{lt}h_j, g_j),
$$
since each $\rm{lt}f_i g_i$ has multidegree $\delta$ while the sum has multidegree $< \delta$. 
...
\end{equation}
where $x^{\gamma_{i,j}} = \rm{lcm}(\rm(lm}(g_i), \rm(lm}g_j)$ it follows that the first sum (Equation \@ref(eq:sum-lt)) is a linear combination of $x^{\delta - \gamma_{i,j}} S(g_i,g_j)$ for appropriate pairs $(i,j)$. 
...
The basic idea of Buchberger's algorithm is that polynomials are added to the input set until a Groebner basis is obtained -- i.e., when Buchberger's criterion is met. 

We first present a worked example to give an ide aof how the algorithm will work.

Consider $I = \langle f_1, f_2 \rangle = \langle x^3 - 2xy, x^2 y - 2y^2 + x \rangle \subset Q[x,y]$ with grlex ordering.
$\langle f_1, f_2 \rangle$ is not a Gr\"obner basis for $I$. 
Indeed, 
\begin{align*}
...
and $\overline{S(f_i,f_j)}^F=0$ for all $f_i,f_j \in F$. We conclude that $F$ is a Gr\"obner basis for $I$. 

We now present the algorithm.

**Input:** $F = \{ f_1,\ldots,f_k \} \subset K[x_1,\ldots,x_n]$ -- a finite set of multivariate polynomials

**Output:** A Gr\"obner basis $G$ for $I = \langle f_1,\ldots,f_k \rangle$, s.t. $F \subset G$
...
::: {.lemma}
If $V \subset K^n$ is an affine variety, then $V(I) \subset K[x_1,\ldots,k_n]$ is an ideal (called the {\em ideal of} $V$). 
:::
...
Now we can return to [@lazard10, Proposition 5.14]. By [@lazard10, Proposition 5.13], be may assume that $\fr(C)$ is the closure of some one-dimensional cells and that blow-up points, if they exist, rae 0-dimensional cylindrical cells in the decomposition induced on $\R^2$. 
...
It is clear that the blow-up poits must have maximum dimension $0$, otherwise the Zariski closure computation will not be able to "fill in the gap". 
