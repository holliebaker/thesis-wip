...
By the proof of @lazard10, Theorem 4.4, we only need to consider points in $\fr{C}$ which project onto a bad cell, say $\mathbf{b}$, of $\mathcal{D'}$.
...
We prove that this result can be extended to cells of dimension $\le 2$ in a sign-invariant CAD of $\R^n$, as long
as the CAD is strong about the projection of the cell. In $\R^n, n > 3$, blow-ups may have dimension greater than $0$.
However, we claim that if a blow-up of dimension $1$ occurs in the frontier of the projection of a cell, the cell tends
to $-\infty$ or $\infty$ as it approaches the blow-up subset.
...
$(i_1,\ldots,i_{n-1},0)$. We actually prove that the frontier of a cell is always contained in a union of cells of smaller binary index, even if the
...
contain a proper subvariety which also has dimension $2$.
If the polynomial which is equal to $0$ at all points of $C$ vanishes identically in a $2$-dimensional cell contained in
$\fr{C'}\times \R$, then $C$ tends to $-\infty$ or $\infty$ as it approaches this cell.
...
Corollary \@ref(cor:ext-lazard-5-3) always holds if the frontier of the projection of the cell in question contains
...
Corollary \@ref(cor:ext-lazard-5-3) fails.
...
This implies that, if Corollary \@ref(cor:ext-lazard-5-3) holds, we should be able to use a similar technique to that described in Proposition \@ref(prp:lazard-5-14)
...
Corollary \@ref(cor:ext-lazard-5-3) since $\dim(C) \le 2$.
