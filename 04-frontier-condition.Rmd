...
This follows from the fact that the Zariski closure of $C$ is an irreducible algebraic variety of dimension $2$ so it cannot
...
<!--This implies that, if Corollary \@ref(cor:ext-lazard-5-3) holds, -->
...
polynomials which define an algebraic set of dimension $2$ the neighbourhood of a point $\mathbf{p}\in
...
As described in the proof of Proposition \@ref(prp:lazard-5-14), $h$ can be taken to be a coefficient of $f$ which is not a multiple of any of $g, g_{j_1}, \ldots,
g_{j_{n-3}}$.
:::
...
$\mathcal{D}$ on $\R^k, k \ge 1$ to be able to apply Lemma \@ref(lem:ext-lazard-5-13) and Corollary \@ref(cor:ext-lazard-5-14).
...
Apply a method similar to that described in @lazard10, Algorithm 5.15 to $C$ and $D$ -- using Corollary
\@ref(cor:ext-lazard-5-14), compute the saturations by $g_{\ell_1}$ and $g_{\ell_2}$, respectively as follows:
\begin{equation}
