...
efficiently [@chen2014]. Another efficient implementation of CAD is available in Mathematica [@strzebonski2006, @strzebonski2016]. However, `QEPCAD-B`, a
...
1. **Input**
    ...
    - *Prenex formula* a first order Boolean formula $(Q_1 v_1), \ldots, (Q_k v_k) F$, where $F$ is a quantifier-free Boolean formula defined by induction as follows.
      - Atoms are polynomial equations, inequations, strict or non-strict inequalities where polynomials are in $\Z[v_1,\ldots,v_n]$.
      - Suppose that quantifier-free Boolean formulas $F_1, F_2$ are defined, then
      $$
...
1. **Normalisation**
    ...
    - Suppose that normalised formulas $G_1,G_1$ are already defined, then
    $$
    G_1 \land G_k \text{ and } G_1 \lor G_k
    $$
...
A sample point of a cell $C$ is a point $\mathbf{x} = (x_1,\ldots,x_k) \in C$ defined over the number field $\Q(\alpha)$.
...
`ADDREFINEMENTPOINTS` is then called, taking these polynomials as input, then add the refinement points above this bad cell.
...
This uses Brown's solution formula construction algorithm for guaranteed solution formula construction.
...
projection factor set. Each cell of $\mathcal{R}$ stores the same information as an ordinary `QEPCAD-B` cells (signs of
...
Let $a = -4\sqrt{2}/3\sqrt{3}$ and $b = 4\sqrt{2}/3\sqrt{3}$ be algebraic numbers. They may be represented as the unique roots of $27x^2 - 32$ in the intervals $(-1115/2^10,-1114/2^10)$ and $(1114/2^10, 1115/2^10)$ respectively. 
