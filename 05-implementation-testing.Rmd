...
Q_{1,3} & = fac(Q_{3,2}) = fac(input)\\
        & = x\\
P_{2,1} & = fac(J_2,1) = fac(dis(A_{3,1}))\\
        & = y^2 + x^2 - 1\\
...
Polynomials and refinement points output by `QEPCAD-B` are presented below and full output appears in Appendix A. Note the projection factor `M_1_3` defines the refinement point $x = 1/2$.
\begin{align*}
...
Example \@ref(exm:whitney-jhd) (@jhd20, Example 2.1), repreduced below, illustrates a CAD which fails to satisfy the frontier condition due to a blow-up point.
...
The projection factors and refinement points produced by `QEPCAD-B` are now presented. An excerpt of the output, including the collection of cells which comprises the frontier of $C$ is presented in Appendix A.
\begin{align*}
\text{Projection polynomials:}\\
...
Additional test cases and benchmarks are presented in Appendix A. TODO!
