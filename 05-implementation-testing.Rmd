...
On lines 77-79, a check to verify whether Conjecture \@ref(cnj:ext-lazard-5-3) is valid. The function `BADCHECK`
determines if there exists a $2$-dimensional sector cell of level $k > 3$ on which a lever-$k$ polynomial vanishes
identically -- i.e., whether there exists a blow-up of dimension $> 0$. In this case, the implementation will fail
gracefully.
Line 88 calls `FRONTIER` to complete the refinement points above bad cells and line 85 calls computes the refinement.
...
\newpage
Polynomials and refinement points output by `QEPCAD-B` are presented below and full output appears in Appendix
...
including the collection of cells which comprises the frontier of $C$ is presented in Appendix \@ref(sec:frontier-output).
\begin{align*}
\text{Projection polynomials:}\\
...
Additional illustrative examples are preesented in Appendix \@ref(sec:more-tests).
Benchmarks are presented in Appendix TODO!
