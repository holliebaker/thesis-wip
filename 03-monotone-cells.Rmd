...
In this case, $\sup_{x_\alpha} f_j \vert_{\{ b_i < x_1 < b_{i+1} \}}$ is independent of $x_1$ and is therefore already monotone.
Now suppose that $Z^{b_i,b_{i+1}}_T \cap Z = \emptyset$.
...
In particular, $C_T \cap \{ b_{i,j} < x_i < b_{i,j+1} \}$ it is the graph of $\mathbf{f}\vert_{Y_T \cap \{ b_{i,j} < x_1 < b_{i,j+1} \}}$. 

Apply the same argument to $\inf_{x_\alpha} f_j$ (for $C_B$) so that a refinement of the kind $\{ b_{i,j} < x_1 < b_{i,j+1} \}$. That it can be assumed that the top and bottom of each two-dimensional section cell is either empty or the graph of a continuous function. 

We now discuss how to perform this refinement on the CAD structure. Let us suppose that $C_T\ne \emptyset$. We must construct subsets $W \subset Y$ such that $f_j\vert_{W_B}$ and $f_j\vert_{W_T}$ are smooth, continuous functions. 
Let 
$$
{\cal A} = ({\cal A}_1, \ldots, {\cal A}_n)
$$
be the projection factor set as defined in Equation \@ref(eq:proj-fac-set). Begin with $j := \alpha + 1$, begin with $j = \alpha + 1$ and let
$$
C' := \projop{j-1}(C), C^j := \projop{j}(C),
$$
Begin with $j = \alpha+1$ so that $C'$ is a cylindrical sector cell and there exists an algebraic variety $V \subset C'_T$, defined in Equation \@ref(eq:c-alpha-top).
By definition, $$
C^j_T = \cl{C^j} \cap (C'_T \times \R)
$$ and there exists a polynomial $g_j \in {\cal A}_j$ such that $g_j(\mathbf{x}) = 0$ for all $\mathbf{x} \in C^j_T$. Since $C^j_T$ is either empty or the graph of a continuous function everywhere except for a finite number of blow-up points, we need to find $x_1$-coordinates of points in $C'_T$ such that $g_j$ is zero over an open interval. 
...
Using $\exists_\infty$, some algebraic computations can be avoided, thereby improving efficiency in practice.**TODO refer to qepcad user documentation.**
...
Now we can assume that $C^j_T\cap \{ b_{i,j} < x_1 < b_{i,j+1}\}$ is the graph of a continuous function. More precisely, the implicit function $g_j$ can be considered as a function from $C'_T \cap \{ b_{i,j} < x_1 < b_{i,j+1}\}$ to $\R$. We have a very similar construction as in Section \@ref(sec:lagrange-refinement) and can apply a very similar construction. 

That is, polynomials $g_2,\ldots,g_{j-1}$ will provide the constraints in Equation \@ref(eq:jacobi-det), while $\partial h / \partial x_i, 1 \le i \le j - 1$ in the last column should be replaced with $\partial g_j / \partial x_i$
...
Suppose that the refinement has been performed for $\alpha + 1 < j$. If $j = n$< we are done, and $C$ is now monotone. Otherwise, let $j := j+1$. By the construction for $\alpha < j$, we know that $C^{j-1}_T \cap \{b_{j-1,i} < x_1 b_{j-1,i+1}\}, 1 \le i < r_{j-1}$ is either empty or the graph of a continuous function. If $C^{j-1}_T = \emptyset$, there is nothing to do, since $C^j_T$ is empty. Otherwise, there are polynomials $g_{j-1} \in {\cal A}_{j-1}$ and $g_j \in {\cal A}_j$ such that $g_{j-1}(\mathbf{x}) = 0$ for all $\mathbf{x} in $C^{j-1}_T$ and $g_j(\mathbf{y}) = 0$ for all $\mathbf{y} in C^j_T$. The same construction as before should be applied. I.e., adding $g_{j-1}$ to the constraints, and using $g_j$ as the optimisation function. We obtain a set of refinement points
$$
...
**Output:** 
$$
{\cal R} := \{ {\cal R}_{\mathbf{b}} = \{ c_1,\ldots,c_t \} \subset \A \mid \mathbf{b} \in \R^{k-1} \text{ is a } (0,\ldots,0) \text{-cell in the decomposition } {\cal D} \text{ induced by } {\cal E} \text{ on } \R^k \}
$$
sucd that intersecting all cells of $\cal D$ with straight lines and half-planes $\{ x_k < c_i \},\{ x_k = c_i \},\{ x_k > c_i \}$ for all $1 \le i \le t$ produces a cad $\cal D'$ which is a refinement of $\cal D$, monotone respect to every cell $C$ of $\cal D$ such that $\dim(C) \le 2$ and $C$ is a *true* cell. 

---

First, define a subroutine
$${\rm sub}_{\mathbf{y}} (g) \mid g \in \Z[y_1,\ldots,y_m,x_{m+1},\ldots,x_n], \mathbf{y} \in \A^m, m < n,$$
which computes $h := g(\mathbf{y}) \in \A[x_{m+1},\ldots,x_m]$ and returns the normalised polynomial of $h$, in $\Z[x_{m+1},\ldots,x_n]$.
...
$$
*(Note: for each $(0,\ldots,0)$-cell $\mathbf{b}$, ${\cal F}_\mathbf{b}$ is a set of univariate polynomials, ${\cal R}_{\mathbf{b}}$ is the set of roots of polynomials ${\cal F}_\mathbf{b}$.)*
...
For each *true* cell $C$ of $\cal D$. If $\dim(C) > 2$, then **fail**. 
Otherwise let $C$ have index
$$
(0,\ldots,0,1,i_{k+1},\ldots,i_{k+n})
$$
in $\cal E$, and let $\alpha$ be the smallest among $x+1,\ldots,k+n$ such that $i_{k+1} = \ldots = i_{\alpha - 1} = 0$ and $i_\alpha = 1$, 
do:
...
- Let
  ...
  - $C_0 := \projop{k-1}(C)$,
    ...
    (*Note: if $k = 1$, then $C_0$ is the unique cell, $\mathbf{0}$, in the CAD of $\R^0$, and ${\rm sub}_\mathbf{0} (g) = g$.*)

  - $\mathbf{b} \in \A^{k-1}$ be the sample point of $C_0$

  - ${\cal G} := {\rm sub}_{\mathbf{b}} \{ (g_{k+1},\ldots,g_{\alpha-1}) \mid k+1 \le i \le \alpha - 1, g_i \in {\cal A}_i, g_i(\mathbf{x}) = 0 \ \forall \mathbf{x} \in \projop{\alpha-1}(C) \}$,

  - $C' := \projop{\alpha}(C)$. 

- $C'_T$ and $C'_B$ be the top and bottom, respectively, of sector cell $C'$. 
  
(*Note: If $C'$ is not bounded from above (resp. below) by a continuous definable function, then let $C'_T := \emptyset$ (resp $C'_B$) and skip any computations involving $C'_T$ (resp $C'_B$).)*
...
$$
  ...
  - If $j < n$, then
    ...
    - If $j \ne \alpha$, let
      ...
      C' := \projop{j}(C)
      $$
      and refine $C'$ into section cells such that $C'_T$ is either empty or the graph of a continuous function. I.e.,
      $$
      ...
      $$
...
- Return the family ${\cal R}$ of refinement poits. 
